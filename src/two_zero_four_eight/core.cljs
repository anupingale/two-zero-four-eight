...
(defn home-page []
  [:div  {:class       "page"
   ...
   [:h2 {:style {:width "400px"}} "Two-0-Four-8"]
...
;This is for terminal version

;(def actions {"L" move-left "R" move-right "U" move-up "D" move-down})

; (filter (complement zero?) (add-valid-adjacents [2 2 4 4]))

;(defn find-empty-pos [v]
;(keep-indexed #(if (zero? %2) %1) v))

;(defn play-2048 [initial-board]
;  (loop [board initial-board]
;    (println board)
;    (let [move (read)]
;      (if ((complement nil?) (some #{2048} (flatten board)))
;        (do
;          (println "YOU WON!!!!!!!!")
;          board)
;        (recur (get-new-board ((actions move) board)))))))
;
